...
library(RHiCDB)
mar<-read.table("2-1_40000.matrix",sep="\t",quote="")
bed<-read.table("2-1_40000_abs.bed",sep="\t",quote="")

head(bed)
head(mar)

circ<-c("chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10","chr11","chr12","chr13","chr14","chr15","chr16","chr17","chr18","chr19")
...
try4<-try4[,c(2,5,7)]
write.table(try4,"chr20.matrix",sep="\t",quote=F,row.names = F,col.names = F)
...
chrsizes='mm10'
outdir='./'
RHiCDB(hicfile,resolution,chrsizes,ref='mm10',outdir=outdir)
